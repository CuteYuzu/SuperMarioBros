...
|------|------|------|
| **HP** | 控制掉血速度（同 osu!std） | - |
| **OD** | 控制 Spiny（蓝色尖刺）的判定框大小 | `1 + 2.9 × OD` 像素 (OD 0 = 1px, OD 10 = 30px) |
| **AR** | 控制小怪出现时间 | `max(1.3, 9 - 0.7 × AR)` 秒 |

> **注意**：已移除 Circle Size，因为不适用于此 Ruleset
...
- **2026-03-20**：v1.0.1 发布，PP 算法优化
- **2026-03-21**：v1.1.0 发布
  - 新增按键框显示（Z/X/←/→ 显示按键状态和次数）
